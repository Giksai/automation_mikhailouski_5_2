...
/**
 * Authenticates current program for google api use
 */
...
/**
 * Gets amount of messages in user's inbox folder
 */
async function getMessagesAmount() {
 ...
 */
async function getMessageSubject(messageId) {
 ...
 * @param {Number} messageId id of a message to get subject from
 */
async function getMessageDeadline(messageId) {
    ...
    if(!body.includes(data.deadlineName)) {
        logger.error("Given message does not contain deadline info!");
        return "";
    }
    let deadline = body.substring(body.indexOf(data.deadlineName));
    deadline = deadline.substring(0, deadline.indexOf(data.deadlineEndName))
    .replace(data.deadlineName, "");
...
module.exports = {
    ...
    getMessageDeadline: getMessageDeadline,
